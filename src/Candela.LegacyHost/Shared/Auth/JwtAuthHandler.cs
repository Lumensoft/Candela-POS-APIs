...
namespace CandelaPOS.Shared.Auth
{
    public class JwtAuthHandler : DelegatingHandler
    {
            ...
            HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            // /legacy/* is the internal channel Candela.Api calls. It carries no cashier
            // session, so a JWT would be meaningless there; LegacySecretHandler guards it
            // with a shared secret instead, and IIS restricts it to 127.0.0.1.
            if (path.IndexOf("/legacy/", StringComparison.OrdinalIgnoreCase) >= 0)
